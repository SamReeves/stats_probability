...
(p_hat <- nrow(twd_no_helmet) / nrow(filter(yrbss, helmet_12m == 'never')))
```
...
```{r}
s_e <- sqrt(p_hat * (1 - p_hat) / nrow(twd_no_helmet))
(m_e <- 1.96 * s_e)
```
...
As $p$ converges with $1 - p$, the value of $ME$ is maximized.  The size of this value is dependent on $n$.
...
The distribution appears normal, centered at (or perhaps just below?) 0.1, from 0.05 to 0.15.  The curve is notable sharp.
...
With $p = 0.3$, the center sits at around 0.3, and the curve becomes flatter, with a spread around 0.18. With $p = 0.5$, the distribution is centered at 0.5, with spread similar to $p = 0.3$.

### 8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

As $n$ increases, the distribution becomes very sharp... Indeed, as $n \rightarrow \infty$, $\hat{p} \rightarrow p$.
...
$H_0 =$ The confidence interval for "sleepers" strength training every day contains the proportion of regular people who do so. 

$H_A =$ The proportions are distinct.
...
Here we reject the null hypothesis and figure that people who sleep 10+ hours a day are more likely to do strength training every day.
...
With a significance level of 0.05, this will happen approximately 5% of the time.
...
$1.96^2 \times \frac{0.5 \times 0.5}{n} < 0.01^{2}$
