...
I would expect other students to get answers for p_hat centered around and averaged at the population proportion.  After consulting with another student, it does seem to be this way.
...
All of these sample proportions hover around the population proportion, as one would expect.  The larger the smaple size, the more the sample proportion will reflect the exact population proportion.
...
```{r}
50 * 15000
m <- mean(sample_props50$p_hat); m
s <- sd(sample_props50$p_hat); s
curve(dnorm(x, m, s),m-(4*s),m+(4*s))
```
...
Each observation represents a group of people polled.  As sample size increases, the sample mean approaches the population mean, and the standard error approaches zero.  The shape of the distribution also normalizes around the mean.
...
```{r}
global_monitor %>%
  rep_sample_n(size = 15, replace = TRUE) %>%
  count(scientist_work) %>%
  mutate(p_hat = n /sum(n)) %>%
  filter(scientist_work == "Benefits")
```

The best point estimate with this sample is 86.7% (run the first time...) Close but not quite right.
...
This sampling is skewed left.  The population mean as per this sample is almost exactly 80%.
...
This is even closer to the population mean!  I would figure 80%.
...
sample_props15 has a considerably large spread than sample_props150.  I would prefer the latter, as the distribution is more regular... It should show the more direct truth about the population proportion.
