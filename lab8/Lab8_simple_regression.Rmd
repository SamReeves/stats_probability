...
2 variables?  I would use a scatter plot.
...
It doesn't look exactly linear, but correlation appears to be high.  I might use it.
...
There is a strong positive linear trend, with correlation much closer to 1 than to 0.  There are a few high-leverage outliers, and it appears that there is no real gain in personal freedom on this metric until about 3.
...
The showSquares = TRUE option isn't working correctly.... I assume I got close with a couple, but I know from past experience that in the presence of high-leverage outliers, you cannot set the line subjectively.
...
```{r}
m2 <- lm(pf_expression_control ~ hf_score, hfi)
summary(m2)
```

\[
y = \beta_0 + \beta_1x + \epsilon = -6.29 + 1.65(x) + 0.5775
\]

The slope indicates a positive linear relationship.
...
They would take the intercept -6.27 and add the x value 6.7 multiplied by the scope to get 4.785.  This would be an underestimate.  It does not account for standard error.
...
It appears that the y values are more scatter at lower x values than at high x values... The variance is therefor not constant, and this is maybe not the best choice for a linear model.
...
```{r}
m3 <- lm(pf_rol ~ pf_expression_jailed, hfi)
summary(m3)

hfi %>% ggplot(aes(x = pf_rol,
                   y = pf_expression_jailed)) +
  geom_point()
```

There appears to be no explanatory relationship....
...
The $R$ and $R^2$ values seem to indicate that there is no relationship really. They are are very close to zero, and so my examination is not a predictive one.
...
```{r}
m4 <- lm(ef_money ~ ef_trade_tariffs, hfi)
summary(m4)

hfi %>% ggplot(aes(x = ef_money,
                   y = ef_trade_tariffs)) +
  geom_point()
```

The relationship between sound monetary policy and tariffs is interesting... Though it is not linear, high tariffs and sound monetary policy seem to go hand in hand.  This appears to say: only sound monetary policy can offer the ability to levy strong taxes.
